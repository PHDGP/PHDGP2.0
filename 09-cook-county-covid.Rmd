...
Since the Cook County Medical Examiner Case Archive data includes records on  deaths where the decedents' residential ZIP codes are inside as well as outside  Cook County, Illinois, we want to restrict our dataset to deaths where their county of residence was in Cook County. 
...
# check how large the population sizes are by race/ethnicity and age
# in each ZCTA
zips_with_population_estimates_by_race_ethnicity_and_age %>% 
  ggplot(
    aes(x = estimate)
...
In the code chunk above we do some exploratory visualizations of the data. The first series of maps show the crude mortality rates by ZCTA, Race/Ethnicity and Age Group.
...
The code above produces a series of maps that visualize the different ABSMs we will use in this analysis.
...
### Differences in COVID-19 mortality rates by race/ethnicity 
...
ggplot() + 
  ...
  scale_y_log10() + 
  ggtitle("Mortality Rates by Race/Ethnicity",
...
ggsave(here("images/09-cook-county-covid/modeled_IRR_by_raceethnicity.png"), width = 8, height = 6)
```
...
overall_models %>%
  ...
  mutate(term_size = as.numeric(stringr::str_extract(term, "[0-9\\.\\-]+"))) %>% 
...
### Gradients in COVID-19 mortality by race/ethnicity in relation to ABSMs
...
In the above examples, we modeled the associations between individual- and area-based measures with the outcome in aggregate across the county. However, we may also be interested in modeling the risk of mortality in each ZCTA. This can help us better understand the spatial distribution of risk. 
...
So, in our dataset can calculate the Observed count $O_i$, the Expected count $E_i$ (based on age standardization), and the SMR $\theta_i$. The observed count approximately follows a Poisson distribution, such that:

$$
O_i \sim Poisson(\theta_i*E_i)
$$

That is, the observed county in any one ZCTA follows a Poisson distribution whose mean is the expected county times a relative risk / SMR. We are interested in this last component since it quantifies the "excess" or "reduced" risk in each area beyond what is "expected" by an age-composition. In our particular case, since we actually have race/ethnicity strata within each ZCTA, we can create a hierarchical structure such that:

$$
O_{ij} \sim Poisson(\theta_{ij}*E_{ij})
$$

Now, we are looking at observed, expected counts and SMR within each race-ethnicity strata $j$, within each ZCTA $i$.

Let us visualize these crude SMRs for each ZCTA-race-ethnicity strata, along with their confidence intervals
...
ggplot(data = smr_df %>% arrange(raw_SMR, desc(Exp)) %>% ungroup() %>% 
  ...
  ylim(0,40) +
  ylab("Crude SMR") +
  xlab("ZCTA-race/ethnicity strata") +
  theme_bw() +
...
However, this model does not have any fixed effects for individual-level variables, so we are estimating the same SMR for each race/ethnicity strata within each ZCTA. We may do just this if we did not have any individual-level data. However, in our case, we are interested in estimating the effect of race-ethnicity, and we can include that in the fixed part of the model such that:
...
One of the motivations for fitting the hierarchical models is to get stable and smoothed estimates of each ZCTA. Let us visualize the modeled SMRs for each ZCTA-race-ethnicity strata, along with their confidence intervals, and compare them to the crude SMRs we calculated in the beginning.
...
The hierarchical models can be most useful when we visualize the estimated ZCTA-specific effects. Since we have race/ethnicity strata within ZCTAs, we can plot maps for each group.
...
smr_df_wnh <- smr_df %>% 
  filter(race_ethnicity == "White, non-Hispanic")
#BYM
formula_bym_mod2 <- Obs ~ 1 + race_ethnicity + ICEraceinc_cut + f(id, model="bym2", graph=INLA_adj_mat, scale.model=TRUE, constr=TRUE)

model_bym_mod2 <- inla(formula_bym_mod2, family="poisson", 
                          data = smr_df, E=Exp,
                          control.predictor=list(compute=TRUE), 
                          control.compute = list(dic = TRUE), verbose = F)
...
#BYM
formula_bym_h2 <- Obs ~ 1 + prop_in_poverty_cut + f(id, model="bym2", graph=INLA_adj_mat, scale.model=TRUE, constr=TRUE)
