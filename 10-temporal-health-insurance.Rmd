...
The goal of this case study is to gain familiarity with the ACS, visualize trends over time, model the effect of different Area-Based Social Metrics (ABSMs) on trends in a particular outcome. The outcome of interest in this case study is health insurance. The specific learning objectives are to: 
Download specific ACS data
Visualize (plot) and map estimates
Characterize trends in the estimates by social variables
...
In this case study, we are particularly interested in observed differences in health insurance coverage across four large states: Massachusetts, California, Texas, and Florida. Our research questions are: 1) What are the trends in health insurance coverage over time in each state and what is the level of certainty in these trends? 2) What is the role of county-level poverty in affecting health insurance coverage over time across counties? And 3) What is the role of county-level income and racial inequality as measured by ICE in affecting health insurance coverage over time across counties?
...
# this is an example where the 5-year estimates look very strange
# 
# we had identified this county by using plotly::ggplotly() on the last map 
health_insurance %>% 
...
# show stat_ellipses for ICEraceinc and poverty 
# 
# TODO: I (Christian) think we should remove this;  it looks pretty, but not sure if it's
# compelling?
# 
# health_insurance %>% 
